...
def img_field(item):
    if item.get('seg_meta', None):
        seg_meta = item["seg_meta"]
        seg_root = item.get("seg_root",None)
        if seg_root is None:
            seg_meta = seg_meta[0]
            image_path=[]
            for path in seg_meta["seg_file"]:
                image_path.append(path)
            res = [path for path in image_path]  
        else:
            seg_meta = seg_meta[0]
            image_path=[]
            for path in seg_meta["seg_file"]:
                ...
                image_path.append(os.path.join(seg_root, path))
            #image_path = [os.path.join(seg_root, path) for path in seg_meta["seg_file"]]
            #print(image_path)
            res = [path for path in image_path]  
    else:
        raise ValueError("No seg_meta or face_seg_path or image_path in item")
...
def load_json(file):
    ...
    with open(file, "r") as f:
        ...
        images = [img_field(item) for item in data]
        prompts = [item["cap"] if 'cap' in item else item["prompts"] for item in data]
    return prompts, images


def save_results(save_path,index):
    if not os.path.exists(save_path):
        os.makedirs(save_path, exist_ok=True)

    if video_length == 1:
        video_path = os.path.join(save_path, f"video_{index}.png")
        ...
        video_path = os.path.join(save_path, f"video_{index}.mp4")
...
# model path
model_name          = "/mnt/bn/bes-mllm-shared/chennan/Code/Open-OmniVCus/DiffSynth-Studio/models/Wan2.2-VACE-Fun-A14B"
...
prompts, images = load_json(args.input_json)
os.makedirs(args.output_dir, exist_ok=True)
for index,prompt, subject_ref_images in zip(range(len(prompts)),prompts, images):
    ...
    if ulysses_degree * ring_degree > 1:
        ...
        if dist.get_rank() == 0:
            save_results(args.output_dir,index)
    else:
        save_results(args.output_dir,index)
